...
exports.default = async function (e) {
  ...
  e.preventDefault();
  try {
    var res = await (0, _axios.post)(this.action);
...
var _google$maps = google.maps,
    ...
    Autocomplete = _google$maps.places.Autocomplete;


var options = { center: { lat: 43.2, lng: -79.8 }, zoom: 10 };
...
var loadPlaces = async function loadPlaces(map) {
  ...
  var res = await fetch('/api/stores/near?lat=' + lat + '&lng=' + lng);
  ...
  var markers = places.map(function (p) {
    ...
    var marker = new Marker({ map: map, position: position });
    marker.place = p;
    return marker;
  });
...
var resultsHTML = function resultsHTML(stores) {
...
exports.default = function (search) {
  if (search) {
    ...
    input.on('input', async function () {
      ...
      results.innerHTML = '';
      try {
        var res = await fetch('/api/search?q=' + this.value);
        var data = await res.json();
        results.innerHTML = _dompurify2.default.sanitize(data.length ? resultsHTML(data) : '<div class="search__result">No results for <mark>' + this.value + '</mark> found!</div>');
      } catch (err) {
        console.error(err);
      }
    });
